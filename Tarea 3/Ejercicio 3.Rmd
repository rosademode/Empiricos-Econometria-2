...
pooled <- plm(log(crmrte) ~ log(prbarr) + log(prbconv) + log(prbpris) + log(avgsen) + log(polpc) + log(wcon) + log(wtuc) + log(wtrd) + log(wfir) + log(wser) + log(wmfg) + log(wfed) + log(wsta) + log(wloc), data = panel, model = "pooling")
...
```{r comment="", echo=FALSE}
firstdiff <- plm(log(crmrte) ~ log(prbarr) + log(prbconv) + log(prbpris)  + log(avgsen) + log(polpc) + log(wcon) + log(wtuc) + log(wtrd) + log(wfir) + log(wser) + log(wmfg) + log(wfed) + log(wsta) + log(wloc), data = panel, model = "fd")
summary(firstdiff)
```
...
```{r comment="", echo=FALSE}
fijos <- plm(log(crmrte) ~ log(prbarr) + log(prbconv) + log(prbpris)  + log(avgsen) + log(polpc) + log(wcon) + log(wtuc) + log(wtrd) + log(wfir) + log(wser) + log(wmfg) + log(wfed) + log(wsta) + log(wloc), data = panel, model = "within")
summary(fijos)
```
...
Al realizar la comparación de los resultados entre el modelo de primeras diferencias y el modelo de efectos fijos, se observan algunos cambios en los coeficientes de las variables de probabilidad asociadas a la justicia penal y en su significancia estadística. 
...
```{r comment="", echo=FALSE}
aleatorios <- plm(log(crmrte) ~ log(prbarr) + log(prbconv) + log(prbpris)  + log(avgsen) + log(polpc) + log(wcon) + log(wtuc) + log(wtrd) + log(wfir) + log(wser) + log(wmfg) + log(wfed) + log(wsta) + log(wloc), data = panel, model = "random")
summary(aleatorios)
```
...
$log(prbarr)$: En el modelo de efectos aleatorios el coeficiente estimado es *(-0.461)*. Por otra parte, en el modelo de efectos fijos el coeficiente estimado es *(-0.226)*. Ambos modelos 

$log(prbconv)$: En el modelo de efectos aleatorios el coeficiente estimado es *(-0.355)*. Por otra parte, el modelo de efectos fijos el coeficiente estimado es *(-0.137)*. 

$log(prbpris)$: En el modelo de efectos aleatorios el coeficiente estimado es *(-0.195)*. Por otra parte, en el modelo de efectos fijos el coeficiente estimado es *(-0.133)*. 
